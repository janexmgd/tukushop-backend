...
const addProductValidation = [
  ...
  check('stock', 'stock value must be more than 1').isInt({
    min: 1,
  }),

  // price
  check('price', 'price required').not().isEmpty(),
  check('price', 'price value must be more than 1').isInt({
    min: 1,
  }),
  ...
  //isNew
  check('isNew', 'isNew required').not().isEmpty(),
  check('isNew', 'isNew value must be boolean').isBoolean(),
